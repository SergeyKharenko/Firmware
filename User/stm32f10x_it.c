...
uint8_t measure_mode=0;				//正在测量标志位
uint32_t frequency=0;				//频率数据

struct Cache_Container Wifi_Cache;      //构建Wifi缓存
...
void TIM2_IRQHandler(void)
{
	...
	else if (TIM_GetITStatus(TIM2, TIM_IT_CC1) != RESET)	//跳变中断
	{
		frequency++;
	}
...
void WIFI_USART_IRQHandler()
{
    ...
    {
        Cache_Add(&Wifi_Cache, (char)USART_ReceiveData(WIFI_USARTx));
    }
